...
import { hostelroomColumns } from "../../datatablesource";
import Datatable from "../../components/datatable/Datatable";
import Sidebar from "../../components/sidebar/Sidebar";
import Navbar from "../../components/navbar/Navbar";
import "./hostelparticular.scss";

import { roomHostelInputs } from "../../formSource";
import { useLocation } from "react-router-dom";
import axios from "axios";
import MetaData from "../../components/layout/Metadata";
import { clearErrors, getRoom } from "../../actions/buildingAction";
...
const HostelParticular = () => {
  ...
  const downloadPDF = () => {
    ...
    if(buildingData && list && list.length > 0){
      const pdf = new jsPDF();
      pdf.text(20, 20, `${buildingData.buildingName} Hostel - Room Data , IIT Jodhpur`, { fontSize: 20, fontStyle: 'bold' });
      
      // Convert data to a format suitable for autotable
      let count=1;
      
      const tableData = list.map(item => [count++,item.buildingName, item.roomNumber+(item.roomDiv==="-"?" ":item.roomDiv), item.roomTitle, item.roomCurrOccu.length,item.roomStatus]);
    
      // Add a table with autotable
      pdf.autoTable({
        startY: 30,
        head: [['Id','Hostel Name', 'Room No.','Room Title','Alloted to No.','Occupancy Status']],
        body: tableData,
      });
  ...
  return (
    <>
    <MetaData title="Hostel-Rooms"/>
    <>
    {
      loading ? (<Loader/>):(
    <div className="hostelparticular-page">
      <Sidebar/>
      <div className="content">
        <Navbar />
         <div className="datatable-container">
          <div className="heading">
              {buildingData && <h1 className="titleColor">{buildingData.buildingName} Hostel Rooms</h1> }   
          ...
          {rooms.roomTitleCount && rooms.occupancyCount && (
           ...
           <Datatable
            columns={hostelroomColumns(handleDelete)}
            rows={list?list:[]}
          />
        </div> 
      </div> 
    ...
    {open && (
        <New inputs={roomHostelInputs} title="Add New Room" link={`/rooms/${path}`} setOpen={setOpen} buildingData={buildingData}/>
      )}
...
export default HostelParticular;
