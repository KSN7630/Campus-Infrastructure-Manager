...
import { amenitiescolumnsroomColumns } from "../../datatablesource";
import Datatable from "../../components/datatable/Datatable";
import Sidebar from "../../components/sidebar/Sidebar";
import Navbar from "../../components/navbar/Navbar";
import "./amenityparticular.scss";
import { amenitiesInputs } from "../../formSource";
...
const Amenityparticular = () => {
  ...
  const downloadPDF = () => {
    ...
    if(buildingData && list && list.length > 0){
      const pdf = new jsPDF();
      pdf.text(20, 20, `${buildingData.buildingName} Amenity- Office/Room/Area Data , IIT Jodhpur`, { fontSize: 20, fontStyle: 'bold' });
      ...
      pdf.autoTable({
        startY: 30,
        head: [['Id','Amenity Name','Office/Room/Area Number','Floor','Office/Room/Area Title','Area Description','Occupancy Status']],
        body: tableData,
      });
    
      pdf.save(`${buildingData.buildingName}_hostel_room_detail.pdf`);
    }  
    else{
  ...
  return (
    <>
    <MetaData title="Amenity-Particular"/>
    <>
    {
      loading ? (<Loader/>):(
    <div className="amenity-particular-page">
      <Sidebar />
      <div className="content">
        <Navbar />
        <div className="datatable-container">
          <div className="heading">
              {buildingData && <h1 className="titleColor">{buildingData.buildingName} Area </h1> }   
             
              {/* {list && <ExportToCSV data={list?list:[]} headers={amenitiescolumnsroomColumns(handleDelete)} filename="amenity_perticular.csv" />}  */}
          ...
          <Datatable
            columns={amenitiescolumnsroomColumns(handleDelete)}
            rows={list?list:[]}
          />
        </div>
      </div>
    ...
    {open && (
        <New inputs={amenitiesInputs} title="Add New Area" link={`/rooms/${path}`} setOpen={setOpen}  buildingData={buildingData}/>
      )}
...
export default Amenityparticular;
