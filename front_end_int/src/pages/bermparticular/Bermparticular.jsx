...
import { bermroomColumns } from "../../datatablesource";
import Datatable from "../../components/datatable/Datatable";
import Sidebar from "../../components/sidebar/Sidebar";
import Navbar from "../../components/navbar/Navbar";
import "./bermparticular.scss";
import { roomInputsBerm } from "../../formSource";
...
const Bermparticular = () => {
  ...
  const downloadPDF = () => {
    ...
    if(buildingData && list && list.length > 0){
      const pdf = new jsPDF();
      pdf.text(20, 20, `${buildingData.buildingName} Berm - Room Data , IIT Jodhpur`, { fontSize: 20, fontStyle: 'bold' });
      ...
      pdf.autoTable({
        startY: 30,
        head: [['Id','Berm No.', 'Office/Lab/Room Number.','Floor','Office/Lab/Room Title','Office/Lab/Room Description' ,'Occupancy Status']],
        body: tableData,
      });
    
      pdf.save(`${buildingData.buildingName}_berm_room_detail.pdf`);
    }  
    else{
  ...
  return (
    <>
    <MetaData title="Berm-Offices"/>
    <>
    {
      loading ? (<Loader/>):(
    <div className="berm-particular-page">
      <Sidebar />
      <div className="content">
        <Navbar />
        <div className="datatable-container">
           <div className="heading">
              {buildingData && <h1 className="titleColor">{buildingData.buildingName} Berm Rooms</h1> }   
              {/* {list && <ExportToCSV data={list?list:[]} headers={bermroomColumns(handleDelete)} filename="berm_perticular.csv" />}  */}
          ...
          <Datatable
            columns={bermroomColumns(handleDelete)}
            rows={list?list:[]}
          />
        </div>
      </div>
    ...
    {open && (
        <New inputs={roomInputsBerm} title="Add New Room" link={`/rooms/${path}`} setOpen={setOpen} buildingData={buildingData}/>
      )}
