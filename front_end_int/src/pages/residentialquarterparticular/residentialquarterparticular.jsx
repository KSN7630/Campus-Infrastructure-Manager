...
import { residentialquarterroomColumns } from "../../datatablesource";
import Datatable from "../../components/datatable/Datatable";
import Sidebar from "../../components/sidebar/Sidebar";
import Navbar from "../../components/navbar/Navbar";
import "./residentialquarterparticular.scss";
import { roomInputsResidential } from "../../formSource";
...
const ResidentialQuarterparticular = () => {
  ...
  const downloadPDF = () => {
    ...
    if(buildingData && list && list.length > 0){
      const pdf = new jsPDF();
      pdf.text(20, 20, `${buildingData.buildingName} Residential Quarter - Room Data , IIT Jodhpur`, { fontSize: 20, fontStyle: 'bold' });

      // Convert data to a format suitable for autotable
      let count=1;
      
      const tableData = list.map(item => [count++,item.buildingName,item.roomNumber+(item.roomDiv==="-"?" ":item.roomDiv), Math.floor(item.roomNumber / 100) - 1,item.roomTitle,item.roomDesc,item.roomStatus]);
    
      // Add a table with autotable
      pdf.autoTable({
        startY: 30,
        head: [['Id','QuarterType Name', 'Flat No.','Floor','Flat Title','Flat Description','Occupancy Status']],
        body: tableData,
      });
    
      pdf.save(`${buildingData.buildingName}_residential_quarter_room_detail.pdf`);
    }
    else{
  ...
  return (
    <>
    <MetaData title="Residential Quarter Flats"/>
    <>
    {
      loading ? (<Loader/>):(
    <div className="residentialquarter-particular-page">
      <Sidebar />
      <div className="content">
        ...
        <div className="datatable-container">
            ...
            <div className="heading">
              {buildingData && <h1 className="titleColor">{buildingData.buildingName} Residential Quarter Flats</h1> }   
              {/* {list && <ExportToCSV data={list?list:[]} headers={residentialquarterroomColumns(handleDelete)} filename="Hostel_Perticular.csv" />}  */}
              {rooms.roomData && <FileUploadComponent linkData={`/rooms/endpoint/dataexel/${path}`} placingOfData="particular"/>} 
            </div>
            <div className="heading">
              <button className="buttonStyle" onClick={()=>{setOpen(true)}}>Add New Flat</button> 
          ...
          <Datatable
            columns={residentialquarterroomColumns(handleDelete)}
            rows={list?list:[]}
          />
        </div>
      </div>
    ...
    {open && (
        <New inputs={roomInputsResidential} title="Add New Flat" link={`/rooms/${path}`} setOpen={setOpen}  buildingData={buildingData}/>
      )}
...
export default ResidentialQuarterparticular;
