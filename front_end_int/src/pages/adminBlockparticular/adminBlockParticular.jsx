...
import { adminblockroomColumns } from "../../datatablesource";
import Datatable from "../../components/datatable/Datatable";
import Sidebar from "../../components/sidebar/Sidebar";
import Navbar from "../../components/navbar/Navbar";
import "./adminBlockParticular.scss";
import { adminblockroomInputs } from "../../formSource";
...
const AdminBlockParticular = () => {
  ...
  const downloadPDF = () => {
    ...
    if(buildingData && list && list.length > 0){
      const pdf = new jsPDF();
      pdf.text(20, 20, `${buildingData.buildingName} - Room Data , IIT Jodhpur`, { fontSize: 20, fontStyle: 'bold' });
      ...
      pdf.autoTable({
        startY: 30,
        head: [['Id','Block Name', 'Office/Room No.','Floor','Office/Room Title','Office/Room Description','Occupancy Status']],
        body: tableData,
      });
  
     pdf.save(`${buildingData.buildingName}_block_room_detail.pdf`);
    }
     else{
  ...
  return (
    <>
    <MetaData title="Admin-Offices"/>
    <>
    {
      loading ? (<Loader/>):(
    <div className="admin-particular-page">
      <Sidebar />
      <div className="content">
        <Navbar />
        <div className="datatable-container">
          <div className="heading">
              {buildingData && <h1 className="titleColor">{buildingData.buildingName} Rooms</h1> }   
              {/* {list && <ExportToCSV data={list?list:[]} headers={adminblockroomColumns(handleDelete)} filename="admin_block_perticular.csv" />}  */}
          ...
          <Datatable
            columns={adminblockroomColumns(handleDelete)}
            rows={list?list:[]}
          />
        </div>
      </div>
    ...
    {open && (
        <New inputs={adminblockroomInputs} title="Add New Room" link={`/rooms/${path}`} setOpen={setOpen} buildingData={buildingData}/>
      )}
...
export default AdminBlockParticular;
