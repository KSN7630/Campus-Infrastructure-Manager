...
import { residentialquartercolumns } from "../../datatablesource";
import Datatable from "../../components/datatable/Datatable";
import Sidebar from "../../components/sidebar/Sidebar";
import Navbar from "../../components/navbar/Navbar";
import "./residentialquarter.scss";
...
import {  buildingQuarterInputs } from "../../formSource";
...
const ResidentialQuarters = () => {
  ...
  const downloadPDF = () => {
    ...
    pdf.text(20, 20, 'Residential Quarter Data - IIT Jodhpur', { fontSize: 20, fontStyle: 'bold' });
    ...
    pdf.autoTable({
      startY: 30,
      head: [['Id','Quarter Type', 'QuarterType Description', 'Total Rooms','Occupied and Locked','Occupied and Open','Vacant']],
      body: tableData,
    });
  
    pdf.save('residential_quarter_data.pdf');
  }
  else{
  ...
  return (
    <>
    <MetaData title="Residential Quarters"/>
    <>
    {
      loading ? (<Loader/>):(
    <div className="residentialquarter-page">
      <Sidebar />
      <div className="content">
        <Navbar />
        <div className="datatable-container">
          <div className="heading">
              <h1 className="titleColor">Residential Quarter</h1>    
              {list && <FileUploadComponent linkData="/building" placingOfData="#"/>} 
          </div>
          <div className="heading">  
              <button className="buttonStyle" onClick={()=>{setOpen(true)}}>Add New Quarter</button> 
              <button className="buttonStyle" onClick={() => downloadPDF()}>Download Detail Summary</button> 
          </div>
          <Datatable
            columns={residentialquartercolumns(handleDelete)}
            rows={list}
            addNewLink="/residentialquarters/new"
          />
        </div>
      </div>
    </div>)}
    ...
    {open && (
        <New inputs={buildingQuarterInputs} title="Add New ResidentialQuarter" link="/building" Part="Residentialquarter" setOpen={setOpen}/>
      )}
...
export default ResidentialQuarters;
